...
import { SLOT_HOURS, formatHour, formatSlotRange } from "@/lib/slots";

type SlotStatus = "available" | "mine" | "booked" | "past";
...
export function WeekCalendar() {
  ...
  function handleCellClick(date: string, hour: number) {
    ...
    if (!slot || slot.status === "past" || slot.status === "booked") return;
    setActionError("");
  ...
  async function handleBook() {
    ...
    const res = await fetch("/api/bookings", {
      ...
      body: JSON.stringify({ date: modal.date, hour: modal.hour }),
    });
    const data = await res.json();
    ...
    <>
      ...
      <div className="flex items-center justify-between mb-4">
        <button
          ...
          className="px-4 py-2 text-sm font-medium text-gray-600 bg-white hover:bg-gray-50 border border-gray-200 rounded-lg shadow-sm transition-all"
        >
          ← Prev week
        </button>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        {(["available", "mine", "booked"] as SlotStatus[]).map((s) => (
          <span key={s} className="flex items-center gap-1.5 text-xs text-gray-600">
            ...
            {s === "available" ? "Available" : s === "mine" ? "My session" : "Taken"}
          </span>
        ))}
      </div>
          ...
          <table className="w-full border-collapse min-w-[600px]">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                <th className="w-20 py-3 px-3 text-right text-xs font-medium text-gray-400">
                  Time
                </th>
                {days.map((date) => {
            ...
            <tbody>
              {SLOT_HOURS.map((hour, idx) => (
                ...
                >
                  <td className="py-1.5 px-3 text-right text-xs text-gray-400 whitespace-nowrap border-r border-gray-100 font-medium">
                    {formatHour(hour)}
                  </td>
                  {days.map((date) => {
                    const slot = getSlot(date, hour);
                    ...
                    return (
                      <td key={date} className="p-1">
                        <button
                          className={`w-full h-9 rounded-lg text-xs font-medium transition-all ${STATUS_CLASSES[status]}`}
                          disabled={disabled}
